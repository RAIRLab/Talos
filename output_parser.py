...
def findSubTokens(statement, transformDict, sorts, quants, quantMap):
    ...
    # If there is a funciton
        ...
        if len(transformDict[args[0]]) > 3:
            argSorts = transformDict[args[0]][3]
            for sort in range(0, len(argSorts)):
                if not args[sort + 1] == "" and not args[sort + 1] in quantMap.keys() and not args[
                            sort + 1] in transformDict.keys():
                    storeQuant(quants, args[sort + 1], argSorts[sort], transformDict, quantMap)
    place = 0
    # Recurse
    for index in range(0, len(args)):
        if args[index] == "":
            args[index] = findSubTokens(temp[sublevel[place][0]:sublevel[place][1]], transformDict, sorts, quants,
                                        quantMap)
            place += 1
    return args
...
def makeFuncs(quants, statementlist, sorts, transformDict, quantMap):
    # Return atomics
    if isinstance(statementlist, string_types):
        returnlist = []
...
def tokenizeSPASS(partialStatement, sorts, transformDict, quantMap):
    ...
    for statement in statements:
        ...
        if tokens[0] == "isValid":
            if isinstance(tokens[1], string_types):
...
if __name__ == "__main__":
    ...
    print(toSNotation(
        "Agent(skc0) Boolean(U) Boolean(Test2BooleanAgent(Boolean(Test1Boolean(a0)),skc0)) isValid(Test2BooleanAgent(Boolean(Test1Boolean(a0)),skc0)) isValid(U)|| -> isValid(U)* -> isValid(U)*.",
        outputContainer.namespace.sorts, transformDict))
    print(toSNotation("|| -> isValid(Agent(James0))*.", outputContainer.namespace.sorts, transformDict))
    print(
        toSNotation("Boolean(U) isValid(V) || isValid(Test1Boolean(U)) -> isValid(U)", outputContainer.namespace.sorts,
                    transformDict))
    print(toSNotation("Boolean(V) isValid(U) Boolean(U) || isValid(Test1Boolean(V)) -> isValid(V)",
                      outputContainer.namespace.sorts, transformDict))
